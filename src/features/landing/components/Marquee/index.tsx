...
export default function Index() {
    return (
        <section className="w-full max-w-7xl m-auto  py-2">
            <Marquee direction={"right"} gradient={true} speed={40}>
                {news.map((item, index) => (

                    <Link dir="rtl" key={index} className="mr-[20px] hover:text-black transition-all border-r border-l px-4 border-primary flex items-center gap-1 text-primary" href={`/news/${item.id}`}>
                        <p>{item.title}</p>
